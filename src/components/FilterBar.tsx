...
interface FilterBarProps {
  ...
  options: { owners: string[]; clientTypes: string[]; industries: string[]; countries: string[]; statuses: string[] }
  resultCount: number
  totalCount: number
}
...
export const FilterBar: React.FC<FilterBarProps> = ({ filters, onChange, options, resultCount, totalCount }) => {
  ...
  const activeFilterCount = [filters.status, filters.owner, filters.clientType, filters.industry, filters.country, filters.dateFrom, filters.dateTo].filter(Boolean).length
  const clearAll = () => onChange({ search: '', status: '', owner: '', clientType: '', industry: '', country: '', dateFrom: '', dateTo: '' })
  ...
  return (
    <div className="space-y-3">
      ...
      {expanded && (
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-2 p-3 bg-base-200 rounded-xl border border-base-300/50">
          ...
          <input type="date" className="input input-bordered input-sm bg-base-100" value={filters.dateFrom} onChange={e => set('dateFrom', e.target.value)} />
          <input type="date" className="input input-bordered input-sm bg-base-100" value={filters.dateTo} onChange={e => set('dateTo', e.target.value)} />
        </div>
      )}
